                               ...
                               unsigned int &outLen)
{
  ...
  return UnPaddingPKCS7(out, inLen, outLen);
}
...
unsigned char* AES::PaddingPKCS7(unsigned char *in, unsigned int inLen, unsigned int &outLen)
{
    outLen = GetPaddingLength(inLen);
    auto paddingSize = outLen - inLen;
    unsigned char *alignIn = new unsigned char[outLen];
    memcpy(alignIn, in, inLen);
    memset(alignIn + inLen, paddingSize, outLen - inLen);
    return alignIn;
}

unsigned char* AES::UnPaddingPKCS7(unsigned char *in, unsigned int inLen, unsigned int &outLen)
{
    auto unpadding = in[inLen-1];
    outLen = inLen - unpadding;
    unsigned char *result = new unsigned char[outLen];
    memcpy(result, in, outLen);
    return result;
}
