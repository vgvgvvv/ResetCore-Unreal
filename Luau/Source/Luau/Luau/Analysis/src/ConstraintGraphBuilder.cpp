...
void ConstraintGraphBuilder::visit(Scope2* scope, AstStatLocal* local)
{
    ...
    for (size_t i = 0; i < local->values.size; ++i)
    {
        ...
        {
            TypeId exprType = lua_check(scope, local->values.data[i]);
            if (i < varTypes.size())
...
{
    lua_check(scope, ifStatement->condition);
...
TypePackId ConstraintGraphBuilder::checkPack(Scope2* scope, AstArray<AstExpr*> exprs)
{
    ...
    {
        if (i < exprs.size - 1)
            types.push_back(lua_check(scope, exprs.data[i]));
        else
...
TypePackId ConstraintGraphBuilder::checkExprList(Scope2* scope, const AstArray<AstExpr*>& exprs)
{
    ...
    for (size_t i = 0; i < exprs.size; ++i)
    {
        ...
        if (i < exprs.size - 1)
            resultPack->head.push_back(lua_check(scope, expr));
        else
...
TypePackId ConstraintGraphBuilder::checkPack(Scope2* scope, AstExpr* expr)
{
    ...
    if (AstExprCall* call = expr->as<AstExprCall>())
    {
        ...
        {
            args.push_back(lua_check(scope, arg));
        }

        // TODO self

        TypeId fnType = lua_check(scope, call->func);
...
TypeId ConstraintGraphBuilder::lua_check(Scope2* scope, AstExpr* expr)
{
    LUAU_ASSERT(scope);
    ...
    if (auto group = expr->as<AstExprGroup>())
        result = lua_check(scope, group->expr);
    ...
    {
        result = lua_check(scope, indexName);
    }
...
TypeId ConstraintGraphBuilder::lua_check(Scope2* scope, AstExprIndexName* indexName)
{
    TypeId obj = lua_check(scope, indexName->expr);
...
TypeId ConstraintGraphBuilder::checkExprTable(Scope2* scope, AstExprTable* expr)
{
    ...
    {
        TypeId itemTy = lua_check(scope, item.value);
        ...
        if (item.key)
        {
            ...
            // astTypes.
            TypeId keyTy = lua_check(scope, item.key);
