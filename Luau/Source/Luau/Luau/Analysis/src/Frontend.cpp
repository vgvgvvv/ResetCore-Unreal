...
LoadDefinitionFileResult loadDefinitionFile(TypeChecker& typeChecker, ScopePtr targetScope, std::string_view source, const std::string& packageName)
{
    ...
    ModulePtr checkedModule = typeChecker.lua_check(module, Mode::Definition);
...
CheckResult Frontend::lua_check(const ModuleName& name, std::optional<FrontendOptions> optionOverride)
{
    ...
    for (const ModuleName& moduleName : buildQueue)
    {
        ...
        if (frontendOptions.forAutocomplete)
        {
            ...
            ModulePtr moduleForAutocomplete = typeCheckerForAutocomplete.lua_check(sourceModule, Mode::Strict);
        ...
        ModulePtr module = FFlag::DebugLuauDeferredConstraintResolution.value ? lua_check(sourceModule, mode, environmentScope)
                                                                        : typeChecker.lua_check(sourceModule, mode, environmentScope);
...
ModulePtr Frontend::lua_check(const SourceModule& sourceModule, Mode mode, const ScopePtr& environmentScope)
{
    ...
    Luau::lua_check(sourceModule, result.get());
