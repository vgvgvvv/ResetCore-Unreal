...
using LuaError = std::optional<std::string>;
...
static LuaError parseBoolean(bool& result, const std::string& value)
{
    if (value == "true")
        result = true;
    else if (value == "false")
        ...
        return LuaError{"Bad setting '" + value + "'.  Valid options are true and false"};

    return std::nullopt;
}

LuaError parseModeString(Mode& mode, const std::string& modeString, bool compat)
{
    if (modeString == "nocheck")
        ...
        return LuaError{"Bad mode \"" + modeString + "\".  Valid options are nocheck, nonstrict, and strict"};

    return std::nullopt;
}

static LuaError parseLintRuleStringForCode(
    LintOptions& enabledLints, LintOptions& fatalLints, LintWarning::Code code, const std::string& value, bool compat)
{
    ...
    else if (compat)
    {
        ...
        {
            return LuaError{"Bad setting '" + value + "'.  Valid options are enabled, disabled, and fatal"};
        }
    }
    else
    {
...
LuaError parseLintRuleString(LintOptions& enabledLints, LintOptions& fatalLints, const std::string& warningName, const std::string& value, bool compat)
{
    if (warningName == "*")
    {
        for (int code = LintWarning::Code_Unknown; code < LintWarning::Code__Count; ++code)
        {
            if (auto err = parseLintRuleStringForCode(enabledLints, fatalLints, LintWarning::Code(code), value, compat))
                return LuaError{"In key " + warningName + ": " + *err};
        }
    }
    else
    {
        ...
        if (code == LintWarning::Code_Unknown)
            return LuaError{"Unknown lint " + warningName};
...
static LuaError fail(Lexer& lexer, const char* message)
{
...
static LuaError parseJson(const std::string& contents, Action action)
{
    Allocator allocator;
    ...
    for (;;)
    {
        if (arrayTop)
        {
            ...
            else if (lexer.current().type == Lexeme::QuotedString)
            {
                ...
                if (LuaError err = action(keys, value))
                    return err;
...
LuaError parseConfig(const std::string& contents, Config& config, bool compat)
{
    return parseJson(contents, [&](const std::vector<std::string>& keys, const std::string& value) -> LuaError {
