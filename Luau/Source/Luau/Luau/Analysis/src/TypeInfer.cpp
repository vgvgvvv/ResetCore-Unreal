...
ModulePtr TypeChecker::lua_check(const SourceModule& module, Mode mode, std::optional<ScopePtr> environmentScope)
{
...
void TypeChecker::lua_check(const ScopePtr& scope, const AstStatBlock& block)
{
...
void TypeChecker::checkBlockWithoutRecursionCheck(const ScopePtr& scope, const AstStatBlock& block)
{
    ...
    for (const auto& stat : sorted)
    {
        ...
        {
            lua_check(scope, *typealias, subLevel, true);
            ++subLevel;
        }
    }
    ...
    auto checkBody = [&](AstStat* stat) {
        if (auto fun = stat->as<AstStatFunction>())
        {
            ...
            lua_check(scope, funTy, funScope, *fun);
        }
...
void TypeChecker::lua_check(const ScopePtr& scope, const AstStatIf& statement)
{
...
void TypeChecker::lua_check(const ScopePtr& scope, const AstStatWhile& statement)
{
    WithPredicate<TypeId> result = checkExpr(scope, *statement.condition);

    ScopePtr whileScope = childScope(scope, statement.body->location);
    resolve(result.predicates, whileScope, true);
    lua_check(whileScope, *statement.body);
}

void TypeChecker::lua_check(const ScopePtr& scope, const AstStatRepeat& statement)
{
...
struct Demoter : Substitution
{
    ...
    {
        return static_cast<bool>(get<FreeTypeVar>(ty));
    }

    bool isDirty(TypePackId tp) override
    {
        return static_cast<bool>(get<FreeTypePack>(tp));
    }
...
void TypeChecker::lua_check(const ScopePtr& scope, const AstStatReturn& return_)
{
...
void TypeChecker::lua_check(const ScopePtr& scope, const AstStatAssign& assign)
{
    std::vector<std::optional<TypeId>> expectedTypes;
...
void TypeChecker::lua_check(const ScopePtr& scope, const AstStatCompoundAssign& assign)
{
...
void TypeChecker::lua_check(const ScopePtr& scope, const AstStatLocal& local)
{
...
void TypeChecker::lua_check(const ScopePtr& scope, const AstStatFor& expr)
{
    ...
    lua_check(loopScope, *expr.body);
}

void TypeChecker::lua_check(const ScopePtr& scope, const AstStatForIn& forin)
{
    ...
    if (forin.values.size == 1 && firstValue->is<AstExprCall>())
    {
        ...
        else if (get<Unifiable::Error>(callRetPack) || !first(callRetPack))
        {
            ...
            return lua_check(loopScope, *forin.body);
        }
        else
        {
    ...
    lua_check(loopScope, *forin.body);
}

void TypeChecker::lua_check(const ScopePtr& scope, TypeId ty, const ScopePtr& funScope, const AstStatFunction& function)
{
...
void TypeChecker::lua_check(const ScopePtr& scope, TypeId ty, const ScopePtr& funScope, const AstStatLocalFunction& function)
{
...
void TypeChecker::lua_check(const ScopePtr& scope, const AstStatTypeAlias& typealias, int subLevel, bool forwardDeclare)
{
...
void TypeChecker::lua_check(const ScopePtr& scope, const AstStatDeclareClass& declaredClass)
{
...
void TypeChecker::lua_check(const ScopePtr& scope, const AstStatDeclareFunction& global)
{
...
void TypeChecker::resolve(const TypeGuardPredicate& typeguardP, RefinementMap& refis, const ScopePtr& scope, bool sense)
{
    ...
    static const std::unordered_map<std::string, SenseToTypeIdPredicate> primitives{
        ...
        {"userdata", mkFilter([](TypeId ty) -> bool { return static_cast<bool>(get<ClassTypeVar>(ty)); })},
    };
    // clang-format on
