...
void AssemblyBuilderX64::extend()
{
    ...
    _code.resize(_code.size() * 2);
    codePos = _code.data() + count;
    codeEnd = _code.data() + _code.size();
}

uint32_t AssemblyBuilderX64::getCodeSize()
{
    return uint32_t(codePos - _code.data());
}
