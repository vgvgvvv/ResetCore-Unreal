...
{
    checkf(LoadHostfxr(), TEXT("Cannot Load Hostfxr lib"));
}
...
load_assembly_and_get_function_pointer_fn DotNetLibManager::GetDotNetLoadAssemblyFunc(const char_t* config_path)
{
    ...
    if (rc != 0 || cxt == nullptr)
    {
        UE_LOG(LogTemp, Error, TEXT("DotNet Init failed:%d"), rc);
        close_fptr(cxt);
        return nullptr;
    }
    ...
    {
        UE_LOG(LogTemp, Error, TEXT("DotNet Get delegate failed:%d"), rc);
    }
